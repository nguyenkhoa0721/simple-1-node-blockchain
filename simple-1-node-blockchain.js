...
class Transaction {
  ...
  async vaild() {
    let isVerified;
    if (this.txOutId == "-1") {
      isVerified = await this.verifySig(this.address, this.signature, this.txOutId);
      if (!isVerified) {
...
app.post("/create", async (req, res) => {
  req.body.txOutId = "-1";
  let ticket = new Ticket(req.body.ticket.name, req.body.ticket.address, req.body.ticket.time);
